...
// V2 CHANGES (System A + G):
// - border-l-4 border-cyan → border-l-2 border-white/25 (subline border)
// - animate-cta-pulse removed from CTA buttons
// - bg-cyan/20 → bg-white/10 on bottom deco lines (vertical lines removed)
// - Static bottom border-line stays (structural separator, not ambient cyan)
...
export function HeroV2({
  ...
  return (
    <section className={cn(
        ...
        <div className="mx-auto max-w-6xl w-full px-4 sm:px-5 md:px-6 lg:px-8 hero-line-reveal">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 lg:gap-12 items-end">
            <div className="lg:col-span-8 min-w-0">
              {/* V2: border-l-2 border-white/25 — cyan ist kein Ambient-Signal mehr */}
                ...
                <div className="mt-6 sm:mt-8 lg:mt-12 flex flex-wrap gap-2 sm:gap-4">
                  {ctas.map((cta, i) => (
                    <Button
                      ...
                      // V2: animate-cta-pulse removed — Hero ist bereits visuell stark genug
                    >
                      <Link href={cta.href}>
                        {cta.label}
                      </Link>
      ...
      {/* Bottom separator — structural only (h-px, white/10 statt cyan/20) */}
      <div className="absolute left-0 right-0 bottom-0 h-px bg-white/10" aria-hidden="true" />
      {/* V2: Vertikale deko-Linien entfernt (waren ambient cyan, kein Interaction-Signal) */}
