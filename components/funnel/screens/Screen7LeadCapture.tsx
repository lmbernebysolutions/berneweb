...
export function Screen7LeadCapture() {
  const { state, submitLead } = useFunnel();
  ...
  const contactKind = useMemo(() => {
    const trimmed = contact.trim();
    if (!trimmed) return "empty" as const;
    if (looksEmail(trimmed)) return "email" as const;
    if (looksPhone(trimmed)) return "phone" as const;
    return "invalid" as const;
  }, [contact]);

  const canSubmit =
    gdprAccepted && (contactKind === "email" || contactKind === "phone") && !isSubmitting;

  const validateBeforeSubmit = (): boolean => {
    if (contactKind === "empty") {
    ...
    if (!gdprAccepted) {
      setGdprError("Bitte zuerst die Einwilligung bestätigen.");
  ...
  const handleWhatsApp = async () => {
    ...
    setSubmitError("");
    const ok = await submitLead(contact.trim(), "whatsapp");
    if (!ok) {
  ...
  const handleEmail = async () => {
    if (!validateBeforeSubmit()) return;
    if (contactKind !== "email") return;

    setSubmitError("");
    const ok = await submitLead(contact.trim(), "email");
    if (!ok) {
      setSubmitError("Fehler beim Senden. Bitte versuch es erneut.");
    }
  };
      ...
      <div className="flex flex-col gap-2">
        <label className="flex items-start gap-3 cursor-pointer group">
          ...
          <span className="text-xs text-white/50 leading-relaxed">
            ...
            <a
              href="/datenschutz"
              className="text-brand-cyan/80 underline underline-offset-2 hover:text-brand-cyan"
              target="_blank"
            >
              Datenschutz
            </a>
        ...
        {gdprError && (
          <p className="text-sm text-red-400 font-medium pl-8">{gdprError}</p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-3 mt-4">
        ...
        {!gdprAccepted && (
          <p className="text-center text-xs text-white/45">
            Bitte zuerst die Einwilligung setzen — danach erscheint der passende
            Button (E-Mail oder WhatsApp).
          </p>
        )}

        {gdprAccepted && contactKind === "phone" && (
          <Button
          ...
          >
            ...
            {isSubmitting ? "WIRD GESENDET..." : "REPORT VIA WHATSAPP"}
          </Button>
        )}

        {gdprAccepted && contactKind === "email" && (
          <Button
            onClick={handleEmail}
            disabled={!canSubmit}
            variant="outline"
        ...
        {gdprAccepted && contactKind === "empty" && (
          <p className="text-center text-xs text-white/40">
            E-Mail → E-Mail-Button · Telefon → WhatsApp-Button
          </p>
        )}

        {gdprAccepted && contactKind === "invalid" && (
