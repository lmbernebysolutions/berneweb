...
// V3 CHANGES (vs ReferenzenCarousel inline):
// - Neutrale Carousel-Dots (bg-white/20, bg-white/40 — kein brand-cyan-Glow auf inaktiven Dots)
// - Tags ohne Cyan-Border/Farbe (neutral: border-white/20 bg-white/5 text-white/60)
// - KPI-Werte in Warm (text-brand-warm statt text-brand-cyan)
// - Blockquote: border-white/25 (statt border-brand-cyan)
// - Übernahme der vollen ReferenzCard Struktur mit nur diesen Delta-Änderungen
...
// =============================================================================
// DEVICE MOCKUPS — identisch mit Original
// =============================================================================
...
// V3: KPI-Wert in Warm, typografisch stark wie Produktion
function ErgebnisBadgeV3({ result }: { result: { wert: string; metrik: string; positiv: boolean } }) {
  return (
    <div className="flex flex-col items-center justify-center border border-white/10 bg-white/5 p-3 text-center">
...
export function ReferenzCardV3({
  referenz,
  featured = false,
  compact = false,
}: {
  ...
  return (
    <article
      className={`group relative overflow-hidden border border-white/15 bg-brand-navy/40 backdrop-blur-md transition-all hover:border-white/30 min-w-0 max-w-full w-full ${
    ...
    >
      ...
      {featured && (
        <div className="absolute top-0 left-1/2 -translate-x-1/2 z-20 border-b border-x border-white/20 bg-white/5 px-6 py-1">
      ...
      <div className={featured ? "lg:grid lg:grid-cols-12 lg:gap-0" : ""}>
        ...
        >
          ...
          {/* V3: Tags neutral — kein Cyan */}
          <div className="relative z-10 mt-6 flex flex-wrap gap-2">
            {referenz.tags.map((tag) => (
              <span
                key={tag}
                className="border border-white/20 bg-white/5 px-2.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-white/60"
              >
                {tag}
              </span>
            ))}
        ...
        <div
          className={`${
            featured ? "lg:col-span-5 lg:border-l lg:border-white/10" : ""
        ...
        >
          ...
          {/* V3: Blockquote border-white/25 statt border-brand-cyan */}
          <blockquote className={cn("border-l-2 border-white/25 pl-4", compact ? "mb-4" : "mb-6")}>
          ...
          {!compact && (
            <div className="mb-6 grid grid-cols-2 gap-2">
              {referenz.ergebnisse.map((r) => (
                <ErgebnisBadgeV3 key={r.metrik} result={r} />
              ))}
            </div>
          )}
...
// CAROUSEL V3
// =============================================================================
export function ReferenzenCarouselV3({ referenzen }: { referenzen: Referenz[] }) {
    ...
    <div className="relative">
      <div className="overflow-hidden" ref={emblaRef}>
        <div className="flex gap-6 md:gap-8">
          {referenzen.map((ref) => (
            ...
            >
              <ReferenzCardV3 referenz={ref} featured={false} />
            </div>
          ))}
        </div>
      </div>
      <div className="mt-8 flex items-center justify-center gap-4">
        <button
          type="button"
          onClick={scrollPrev}
          className="flex h-12 w-12 items-center justify-center border border-white/20 bg-white/5 text-white transition-all hover:border-white/40 hover:bg-white/10"
          ...
          {referenzen.map((_, i) => (
            <button
              ...
              className={`h-2 shrink-0 rounded-full transition-all ${
                i === selectedIndex
                  // V3: aktiver Dot bleibt weiß (kein cyan-Leuchten auf inaktiven)
                  ? "w-6 bg-white"
